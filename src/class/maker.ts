...
export class Maker {
  ...
  private writeFileAsync: (path: string, data: string | Buffer, options?: fs.WriteFileOptions) => Promise<void>
  ...
  private summaryFileSectionHeadingLevel = 1
  ...
  async applySummaryMore(
    ...
    if (headingLevel) {
      this.summaryFileSectionHeadingLevel = headingLevel
    }
  ...
  private async readFilesFromFolder(
    ...
    try {
      ...
      readFilesList
        .map(async(baseNameReadFile: string) => {
          ...
          if (stat && stat.isDirectory()) {
            this.readFilesFromFolder(readFile, (_: Error | null, readData: string[]) => {
      ...
      if (readFilesFromFolderError instanceof Error) {
        onReadFilesFromFolder(readFilesFromFolderError, [])
      } else {
        onReadFilesFromFolder(new Error('Unknown error in readFilesFromFolder'), [])
      }
    }
  }
  ...
  private updateSummaryFile(readError: Error | null, readFiles: string[]): void {
    if (readError) {
    ...
    readFiles = readFiles.map((readFile: string) => {
      ...
      const documentName = path.parse(filePath).name
      const documentFolder = path.parse(filePath).dir ? ` (${path.parse(filePath).dir.replace('/', ' / ')})` : ''
      const documentLink = filePath.replace(/[ ]/g, '&#32;')

      return `* [${documentName}${documentFolder}](${documentLink})`
    })
  ...
  private checkExistingSection(line: string, index: number): boolean {
    ...
    if (regExpr.test(line)) {
      this.existingSectionIndex = index
      this.summaryFileSectionHeadingLevel = line.split('#').length - 1
      return true
    }
    return false
  }
  ...
  private removeExistingSection(_: string, index: number): boolean {
  ...
  private async handleSummaryContent(readLines: string[], documentLines: string[]): Promise<void> {
    ...
    if (!isExistingSection) {
      ...
      documentLines.splice(0, 0, this.autogeneratedMessage, `${headingLevel} ${this.summaryFileSection}`)
    } else {
      const autoGeneratedRegExp = new RegExp(/^(\*- This summary is autogenerated by)/)

      if (autoGeneratedRegExp.test(readLines[this.existingSectionIndex - 1])) {
        documentLines.splice(0, 0, `${headingLevel} ${this.summaryFileSection}`)
      } else {
        documentLines.splice(0, 0, this.autogeneratedMessage, `${headingLevel} ${this.summaryFileSection}`)
      }
    }
    ...
    documentLines.push('', this.autogeneratedMessage)
    readLines.splice(
      this.existingSectionIndex,
      this.existingSectionNextIndex - this.existingSectionIndex,
      ...documentLines
    )
